    ...
    while True:
        ...
        for alien in aliens:
            if alien.state == Alien.ALIVE:
                ...
                if laser.active and intersect(laser.x, laser.y, laser.width, laser.height, alien.x, alien.y, width,
                                              height):
                    # laser hit an alien
        ...
        if not at_least_one_alien_alive:
            # No aliens remaining_ms => send a new wave
            aliens = init_aliens(ALIENS_ROWS, ALIENS_COLS, ALIENS_INIT_X, ALIENS_INIT_Y, ALIENS_SPACING_X,
                                 ALIENS_SPACING_Y)
